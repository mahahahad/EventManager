    ...
    return (
        <div className="relative min-h-screen text-white pt-28 pb-12 px-4 sm:px-6 lg:px-8 bg-black overflow-hidden">
            ...
            <div className="relative z-10 px-6 py-8 max-w-4xl mx-auto space-y-6">
                <h2 className="text-3xl font-bold tracking-tight mb-6 text-transparent bg-clip-text bg-gradient-to-r from-gray-200 via-white to-gray-300">
                    Your Dashboard
                </h2>

                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    {/* Calendar Card */}
                    <Card className="bg-black/30 backdrop-blur-lg shadow-lg rounded-xl border border-black/40 dark:bg-gray-800/70 dark:border-gray-600">
                        <CardHeader>
                        ...
                        <CardContent className="p-4">
                            <Calendar
                                ...
                                classNames={{
                                    day: "h-9 w-9 p-0 font-normal text-center text-base rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-1 transition-colors duration-200 ease-in-out hover:bg-neutral-700 text-white",
                                    day_selected: "bg-blue-500 text-white focus:ring-2 focus:ring-blue-500 focus:ring-offset-1",
                                ...
                                modifiersClassNames={{
                                    hasEvent: "relative before:content-[''] before:absolute before:bottom-0 before:left-1/2 before:-translate-x-1/2 before:h-1 before:w-1 before:rounded-full before:bg-blue-500",
                                }}
                            />
                            {selectedDate && (
                                ...
                                <ul className="space-y-3">
                                    {eventsOnSelectedDate.map((event) => (
                                        <li key={event.id} className="flex items-center justify-between">
                                            <Link href={`/events/${event.id}`} className="hover:underline text-blue-400">
                                                {event.title}
                                            </Link>
